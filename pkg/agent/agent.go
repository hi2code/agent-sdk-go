...
func (a *Agent) RunMultiContent(ctx context.Context, input string, inputMultiContent []interfaces.MultiContentPart) (string, error) {
	response, err := a.runInternalMultiContent(ctx, input, false, inputMultiContent)
	if err != nil {
		return "", err
	}
	return response.Content, nil
...
func (a *Agent) runInternalMultiContent(ctx context.Context, input string, detailed bool, inputMultiContent []interfaces.MultiContentPart) (*interfaces.AgentResponse, error) {
	startTime := time.Now()
	...
	if a.customRunFunc != nil {
		...
		response, err = a.runLocalWithTrackingMultiContent(ctx, input, inputMultiContent)
		if err != nil {
			return nil, err
		}
	}
...
func (a *Agent) runLocalWithTrackingMultiContent(ctx context.Context, input string, inputMultiContent []interfaces.MultiContentPart) (string, error) {
	...
	if a.memory != nil {
		if err := a.memory.AddMessage(ctx, interfaces.Message{
			...
			Content:      input,
			MultiContent: inputMultiContent,
		}); err != nil {
	...
	return a.runWithoutExecutionPlanWithToolsTrackedMultiContent(ctx, input, allTools, inputMultiContent)
}

func (a *Agent) runWithoutExecutionPlanWithToolsTrackedMultiContent(ctx context.Context, input string, tools []interfaces.Tool, inputMultiContent []interfaces.MultiContentPart) (string, error) {
	prompt := input
	...
	if len(tools) > 0 {
		...
		if tracker != nil && tracker.detailed {
			...
			response, err = a.llm.GenerateWithToolsMultiContent(ctx, prompt, inputMultiContent, tools, generateOptions...)
			if err != nil {
				return "", fmt.Errorf("failed to generate response: %w", err)
			}
		}
